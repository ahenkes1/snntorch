...
def _layer_check(net):
    ...
    for idx in range(len(list(net._modules.values()))):
        ...
        if isinstance(list(net._modules.values())[idx], snn.Leaky):
            is_leaky = True
        if isinstance(list(net._modules.values())[idx], snn.ReluLeaky):
            is_reluleaky = True
...
def _layer_reset():
    ...
    if is_reluleaky:
        snn.ReluLeaky.reset_hidden()  # reset hidden state to 0's
        snn.ReluLeaky.detach_hidden()
    if is_alpha:
...
def _final_layer_check(net):
    ...
    if isinstance(list(net._modules.values())[-1], snn.Leaky):
        return 2
    if isinstance(list(net._modules.values())[-1], snn.ReluLeaky):
        return 2
